...
const ReservePageFromSideBar = ({ setBgColor }) => {
  ...
  useEffect(() => {
    const fetchCars = async () => {
      try {
        const response = await fetch('https://driver-app-ezqp.onrender.com/api/cars', {
          headers: {
  ...
  const handleReservation = async (e) => {
    ...
    try {
      const response = await fetch(`https://driver-app-ezqp.onrender.com/api/cars/${selectedCarId}/reservations`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
